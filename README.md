...
44.  Java IO流 根据数据的处理方式又分为字节流和字符流。 缓冲流：在JVM中开辟一块缓冲区的内存空间，将文件中的数据先读取到缓冲区中，直到缓冲区满了，才将数据传递给程序。

45.  Java中参数传递的方式是值传递，不是引用传递。

46. Unsafe 是一个类，主要提供一些用于执行低级别、不安全操作的方法，如直接访问系统内存资源、自主管理内存资源

47. Java集合： Collection 和 Map。 接口Collection 接口下面的集合：List，Set，Queue。
             ArrayList: 内部基于动态数组实现,创建时不需要指定大小,只能存储对象
             Map：HashMap（线程不安全）就是用了拉链法的哈希表，当链表长度太长（默认超过8）时，链表就转换为红黑树。

48. 并发：两个及两个以上的作业在同一 时间段 内执行。 并行：两个及两个以上的作业在同一 时刻 执行

49. 变量的可见性：是指一个线程对共享变量的修改，另一个线程可以感知到。每个CPU都有自己的缓存，多个线程在不同的CPU中处理数据就会导致不可见问题。使用volatile关键字可以让线程每次使用它都到主存中进行读取。
50. 指令重排序：JVM为了使处理器内部的运算单元能尽量被充分利用，可能会对输入的代码进行乱序执行优化，确保这一结果和顺序执行结果是一致的。使用volatile关键字可以通过 内存屏障 来禁止指令重排序。Unsafe类也可以。
51. 内存屏障：是指保护指令按照顺序执行的机制。
52. 并发编程中的原子性：指一组操作要么完全执行，要么不执行。这可以有利于多个线程对共享资源访问时的安全性。

53. 同步：是指一个进程在执行某个请求的时候，如果该请求需要一段时间才能返回信息，那么这个进程会一直等待下去，直到收到返回信息才继续执行下去。 synchronized关键字可以保证被它修饰的方法或者代码块在任意时刻只能有一个线程执行。
54. 异步：是指进程不需要一直等待下去，而是继续执行下面的操作，不管其他进程的状态，当有信息返回的时候会通知进程进行处理，这样就可以提高执行的效率了12。
    
55. 悲观锁：共享资源每次只给一个线程使用，其它线程阻塞，用完后再把资源转让给其它线程。可能会造成性能下降和死锁。多用于写比较多的情况。
56. 乐观锁：操作数据时不会进行加锁，只有到数据提交的时候才通过一种机制来验证数据是否存在冲突，存在冲突就需要重试。多用于写少读多的情况。

57. 公平锁：锁被释放之后，先申请的线程先得到锁。性能差一点。
58. 非公平锁：按照优先级排序，优先级高的线程先得到锁。性能好，但可能会导致某些线程永远无法获取到锁。

59. 递归锁：指线程得到某个锁后，在释放前还可以再次得到这个锁。非递归锁的话，会造成死锁。

60. 共享锁：一把锁可以被多个线程同时获得。
61. 独占锁：一把锁只能被一个线程获得。t

62. ThreadLocal类：让一个线程有自己的专属本地变量
63. 线程池：就是管理一系列线程的资源池。（通过ThreadPoolExecutor构造函数来创建）
    好处：-降低资源消耗。通过重复利用已创建的线程降低线程创建和销毁造成的消耗。
          -提高响应速度。当任务到达时，任务可以不需要等到线程创建就能立即执行。
          -提高线程的可管理性。线程是稀缺资源，如果无限制的创建，不仅会消耗系统资源，还会降低系统的稳定性，使用线程池可以进行统一的分配，调优和监控。

   ![图片](https://github.com/Jonas9172/JavaNotes/assets/105164575/cb6d2339-948c-43fe-9b17-7c627ac284ec)

64. Java内存模型：
    ![图片](https://github.com/Jonas9172/JavaNotes/assets/105164575/38f0328f-ad20-405e-8f31-6595791a6ee8)

65. IO 模型
   同步阻塞 IO 模型
   ![图片](https://github.com/Jonas9172/JavaNotes/assets/105164575/98cead9d-9709-41b0-ac45-ac0c86c6d845)

   同步非阻塞 IO 模型
   ![图片](https://github.com/Jonas9172/JavaNotes/assets/105164575/8ce9e340-8b42-4df0-b5b4-2301a5db93f7)

66. JVM运行时数据区：
    程序计数器：一块较小的内存空间，可以看作是当前线程所执行的字节码的行号指示器，它指向下一条需要执行的字节码指令，比如循环、异常处理等。多线程的情况下还需要记录该线程上次运行到哪了。
    虚拟机栈：每一次方法调用都会有一个栈帧入栈，调用结束后对应栈帧被弹出。 栈帧包括局部变量表、操作数栈、动态链接、方法出口。
